...
export default function DeckManager() {
  ...
  return (
    <div className="flex flex-col h-dvh bg-[var(--app-bg)] items-center justify-center p-8 text-center">
      <p className="text-5xl mb-4">📖</p>
      <h1 className="text-2xl font-bold text-[var(--ink)] mb-2">Vocabulary Trainer</h1>
      <p className="text-[var(--ink-soft)] mb-8 text-sm">Create your first deck to get started.</p>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--surface)] rounded-2xl w-full max-w-sm p-5 shadow-xl" style={{ border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-[var(--ink)]">New deck</h2>
          <button onClick={onCancel} className="text-[var(--ink-faint)] hover:text-[var(--ink)] text-xl leading-none">×</button>
        </div>
        <div className="flex flex-col gap-3">
          <div>
            <label className="block text-sm text-[var(--ink-soft)] mb-1">Deck name</label>
          ...
          <div>
            <label className="block text-sm text-[var(--ink-soft)] mb-1">Language you're learning</label>
            <input value={target} onChange={(e) => setTarget(e.target.value)} onKeyDown={onEnter} placeholder="e.g. de or German" className="input" />
          </div>
          <div>
            <label className="block text-sm text-[var(--ink-soft)] mb-1">Your native language</label>
